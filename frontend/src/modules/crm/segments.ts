...
export function buildLifecycleScenarios(context: SegmentContext): string[] {
  ...
  if (context.campaignEligibilityState === 'prioritized') scenarios.push('member_priority_campaign')
  if (context.campaignEligibilityState === 'cooldown') scenarios.push('offer_cooldown')
  if (context.seasonalEligibilityState === 'highlighted') scenarios.push('seasonal_perk_highlight')
  if (context.seasonalEligibilityState === 'teaser') scenarios.push('seasonal_teaser')
  if (context.recommendationState === 'reactivation') scenarios.push('reactivation_journey')
